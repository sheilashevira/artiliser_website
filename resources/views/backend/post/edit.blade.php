...
<form action="/post/update/{{ $posts->id }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="-mx-3 md:flex sm:mb-2">
      <div class="md:w-full px-3">
        ...
        <input id="name" name="name" value="{{ $posts->name}}" class="w-full bg-grey-lighter border text-sm text-grey-darker py-3 px-4 rounded mt-1 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent" style="border-color:#670067" id="grid-name" type="text" placeholder="Input Post Name">
  ...
  <div class="-mx-3 md:flex sm:mb-2">
    <div class="md:w-1/2 px-3">
      ...
      <select id="category_id" name="categories_id" onchange="changeCategory()" class="w-full bg-grey-lighter border text-sm text-grey-darker py-3 px-4 rounded mt-1 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent" style="border-color:#670067">
        <option value="{{ $posts->categories->id}}">{{ $posts->categories->name}}</option>
    ...
    <div class="md:w-1/2 px-3 sm:mb-6 mt-6 sm:mt-0">
      ...
      <option value="{{ $posts->carousel_rows->id }}">{{ $posts->carousel_rows->name }}</option>
  ...
  <div class="c2 div" id="c2_div">
    <div class="-mx-3 md:flex mb-6 mt-6 sm:mt-0">
      <div id="printmaterials" class="md:w-1/2 px-3" hide>
        ...
        <select id="print_materials_id" name="print_materials_id"  class="w-full bg-grey-lighter border text-sm text-grey-darker py-3 px-4 rounded mt-1 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent" style="border-color:#670067">   
      ...
      <div class="md:w-1/2 px-3 mt-6 sm:mt-0">
        ...
        <select id="sizes_id" name="sizes_id"  class="w-full bg-grey-lighter border text-sm text-grey-darker py-3 px-4 rounded mt-1 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent" style="border-color:#670067">
          <option value="{{ $posts->sizes->id}}">{{ $posts->sizes->name}}</option>
      ...
      <div class="md:w-1/2 px-3 mt-6 sm:mt-1">
        ...
        <input id="price" name="price" value="{{ $posts->price}}" class="appearance-none block w-full bg-grey-lighter text-sm text-grey-darker border rounded py-3 px-4 mb-3" style="border-color:#670067" id="grid-price" type="text" placeholder="Input Price">
          <div class="text-red-600"> 
            @error('price')
                {{ $message}}
            @enderror
  ...
  <div class="-mx-3 md:flex mb-6 mt-4 sm:mt-0">
    <div class="md:w-full px-3">
      ...
      <input id="image-input" name="image" value="{{ $posts->image}}" class="py-2 mb-3" type="file" accept="image/*">
      <label for="image-input" >
        <img id="preview-image" width="300px" src="/img/{{ $posts->image }}" alt="">
          <div class="text-red-600"> 
            @error('image')
                {{ $message}}
            @enderror
  ...
  <div class="-mx-3 md:flex mb-6">
    <div class="md:w-full px-3">
      ...
      <textarea id="content" name="content" class="border-2 border-gray-500">{{ $posts->content}}</textarea>
        <div class="text-red-600"> 
            @error('content')
                {{ $message}}
            @enderror
